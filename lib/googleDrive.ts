...
function rootFolderName() { return import.meta.env.VITE_GOOGLE_DRIVE_ROOT_FOLDER || 'Garagem SaaS' }
...
export function isGoogleDriveConfigured() { return !!clientId() }
...
async function token() { return accessToken || await connectGoogleDrive() }
...
async function createFolder(name: string, parentId?: string) {
  ...
  const res = await driveFetch('https://www.googleapis.com/drive/v3/files?fields=id,name', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) })
  const data = await res.json()
  return data.id
}
async function folder(name: string, parentId?: string) { return (await findFolder(name,parentId)) || await createFolder(name,parentId) }
async function makePublic(fileId: string) {
  await driveFetch(`https://www.googleapis.com/drive/v3/files/${fileId}/permissions`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ role:'reader', type:'anyone' }) })
}
...
export async function uploadFileToGoogleDrive(file: File, opts?: { clientName?: string; osNumber?: string }) {
  ...
  const metadata = { name: safeFileName(file.name), parents: [os], mimeType: file.type || 'application/octet-stream' }

  const init = await driveFetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=resumable&fields=id,name,webViewLink,webContentLink', {
    method:'POST',
  ...
  if (!uploadUrl) throw new Error('Google Drive não retornou URL de upload.')
  ...
  return { id:data.id, name:data.name, viewLink:data.webViewLink || `https://drive.google.com/file/d/${data.id}/view`, downloadLink:data.webContentLink || `https://drive.google.com/uc?id=${data.id}&export=download`, folderId: os }
}
