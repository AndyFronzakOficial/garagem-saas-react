...
// Gera um ID local seguro para cada item do PDV.
// Evita tela branca em navegadores/ambientes onde crypto.randomUUID não existe.
function createLocalId() {
  try {
    ...
    // Se o navegador bloquear crypto, usa o fallback abaixo.
  }

  return `item-${Date.now()}-${Math.random().toString(36).slice(2)}`;
}

// Lê o tema salvo sem quebrar a tela caso o navegador bloqueie localStorage.
...
// Salva o tema sem travar o PDV caso o navegador bloqueie localStorage.
function saveTheme(theme: "dark" | "light") {
  try {
    localStorage.setItem("pdv-theme", theme);
  } catch {
    // Não faz nada: o tema continua funcionando na tela atual.
  }
}
...
// Retorna a unidade correta da precificação para exibir no PDV.
function pricingUnit(service: any) {
  return service?.pricing_type === "quantity" ? "un" : "m²";
}

// Identifica se o serviço selecionado deve ser calculado por quantidade/unidade.
function isQuantityService(service: any) {
  return service?.pricing_type === "quantity";
}
...
export default function PublicQuote() {
  ...
  async function getOrCreateClientId() {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error)
      throw new Error("Erro ao cadastrar cliente no PDV: " + error.message);
  ...
  function calcItem(item: QuoteItem) {
    ...
    const quantity = Math.max(brNumber(item.quantity) || 1, 1);
    const unitPrice = Number(service?.price_m2_final || 0);

    // Se o serviço for por quantidade, calcula unidade x quantidade.
    // Se for por m², calcula área x quantidade x valor do m².
    const autoTotal = isQuantityService(service)
      ? quantity * unitPrice
      : area * quantity * unitPrice;
  ...
  const total = useMemo(
    () => items.reduce((sum, item) => sum + calcItem(item).total, 0),
    [items, services],
  );
  ...
  async function uploadProjectImage(file: File, code: string) {
    ...
    if (error)
      throw new Error(
        "Erro ao enviar imagem do projeto para o Supabase: " + error.message,
      );
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      const normalizedItems = items.map((item, index) => {
        ...
        if (!calc.service)
          throw new Error(`Selecione o serviço do item ${index + 1}.`);
        if (
          !isQuantityService(calc.service) &&
          (!item.width_cm || !item.height_cm)
        )
          throw new Error(`Informe as medidas do item ${index + 1}.`);

        return {
          service_price_id: calc.service.id,
          service_name: calc.service.name,
          pricing_type: calc.service.pricing_type || "m2",
      ...
      const { error } = await supabase.from("public_quotes").insert({
        ...
        area_m2: normalizedItems.reduce(
          (sum, item) => sum + Number(item.area_m2 || 0),
          0,
        ),
        price_m2: first.price_m2,
        auto_estimated_price: normalizedItems.reduce(
          (sum, item) => sum + Number(item.auto_estimated_price || 0),
          0,
        ),
        manual_price: null,
      ...
      >
        <div className="client-shell grid min-h-screen place-items-center">
          <div className="card text-center w-full max-w-lg">
            ...
            {sent.project_name && (
              <p className="muted-text">Projeto: {sent.project_name}</p>
            )}
            <p className="muted-text">Código: {sent.code}</p>
            <p className="muted-text">
              Serviços cadastrados: {sent.items?.length || 1}
            </p>
            <div className="price-preview mt-4">
              <strong>{money(sent.total)}</strong>
            </div>
            {sent.fileUrl && (
              <a
                href={sent.fileUrl}
                target="_blank"
                className="mt-4 block text-gold"
              >
                Arquivo enviado para o Drive
              </a>
            )}
            {sent.projectImageUrl && (
              <a
                href={sent.projectImageUrl}
                target="_blank"
                className="mt-2 block text-gold"
              >
                Imagem do projeto enviada
              </a>
            )}
          </div>
        </div>
    ...
    >
      <header className="portal-topbar">
        <div className="portal-topbar-inner">
          <img
            src="/logo.png"
            alt="Garagem Comunicação Visual"
            className="logo-img max-h-12 object-contain"
          />
          <button type="button" className="btn-dark" onClick={toggleTheme}>
      ...
      <div className="client-shell portal-shell-offset">
        <section className="client-card">
          ...
          <p className="muted-text">
            Informe medidas para serviços por m² ou apenas quantidade para
            serviços por unidade.
          </p>

          {errorMsg && (
            <div className="mt-4 rounded-xl border border-red-500/30 bg-red-950/60 p-4 text-red-100">
              {errorMsg}
            </div>
          )}
          ...
          <form onSubmit={save} className="client-grid mt-6">
              ...
              <div className="mb-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="text-sm muted-text">
                    Selecione um cliente já cadastrado para alimentar o
                    histórico dele ou cadastre um novo por aqui.
                  </p>
                </div>
                <div className="flex gap-2">
                  <button
                    type="button"
                    className={
                      clientMode === "existing" ? "btn-gold" : "btn-dark"
                    }
                  ...
                  <button
                    type="button"
                    className={clientMode === "new" ? "btn-gold" : "btn-dark"}
                    onClick={() => {
                      setClientMode("new");
                      setSelectedClientId("");
                      setClientSearch("");
                  ...
                  <div className="grid gap-2 md:grid-cols-2">
                    {filteredClients.map((c) => (
                      <button
                        ...
                        className={`rounded-xl border p-3 text-left transition ${selectedClientId === c.id ? "border-gold/50 bg-gold/10" : "border-white/10 bg-black/10 hover:bg-gold/10"}`}
                      >
            ...
            <input
              className="input full"
              placeholder="Nome do projeto. Ex: Fachada Loja Central"
              value={f.project_name}
              onChange={(e) => setF({ ...f, project_name: e.target.value })}
            />
                  ...
                  <div key={item.localId} className="portal-inner-card p-4">
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      ...
                      <input
                        ...
                        value={item.width_cm}
                        onChange={(e) =>
                          updateItem(item.localId, { width_cm: e.target.value })
                        }
                        required={!isQuantityService(calc.service)}
                      />
                      <input
                        className="input"
                      ...
                      <input
                        className="input md:col-span-2"
                        placeholder="Valor deste serviço opcional. Ex: 250,00"
                      ...
                      <textarea
                        className="input md:col-span-2"
                        placeholder="Observação deste serviço"
                    ...
                    <div className="portal-inner-card mt-3 p-3 text-sm muted-text">
                      ...
                      • Automático: {money(calc.autoTotal)} •{" "}
            ...
            <div className="full">
              <label className="mb-2 block text-sm muted-text">
                Imagem do projeto para aparecer no PDF da OS
              </label>
              <input
                className="input"
                type="file"
              ...
              <p className="mt-2 text-xs muted-text">
                Opcional. Envio direto pelo Supabase, limite de 50MB.
              </p>
              {projectImage && (
                <p className="mt-2 text-sm muted-text">
                  Imagem selecionada: {projectImage.name}
                </p>
              )}
            </div>
