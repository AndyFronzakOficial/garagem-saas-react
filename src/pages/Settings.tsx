...
export default function Settings(){
  ...
  const [f,setF]=useState({company_name:'Garagem Comunicação Visual',cnpj:'36.685.414/0001-49',address:'R. Califórnia, 287 - Guaraituba, Colombo - PR, 83410-140',phone:'(41) 99267-5409',logo_url:'/logo.png',monthly_goal:'35000',notify_os_new:true,notify_os_ready:true,notify_overdue:true,notify_installation:true})

  useEffect(()=>{load()},[])
  async function load(){
    const {data}=await supabase.from('company_settings').select('*').eq('id',1).maybeSingle()
    if(data) setF({company_name:data.company_name||f.company_name,cnpj:data.cnpj||f.cnpj,address:data.address||f.address,phone:data.phone||f.phone,logo_url:data.logo_url||'/logo.png',monthly_goal:String(data.monthly_goal||'35000'),notify_os_new:data.notify_os_new!==false,notify_os_ready:data.notify_os_ready!==false,notify_overdue:data.notify_overdue!==false,notify_installation:data.notify_installation!==false})
  }
