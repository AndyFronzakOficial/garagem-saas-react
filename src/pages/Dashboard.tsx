...
export default function Dashboard(){
 ...
 return <div className="page-fade">
  ...
  <section className="grid gap-4 md:grid-cols-3 xl:grid-cols-6">{[['Faturamento',s.faturamento],['Recebido',s.recebido],['Despesas',s.despesas],['Saldo real',s.saldo],['Meta',s.meta],['Lucro estimado',s.lucro],['Ticket médio',s.ticket]].map(([n,v])=><article className="metric-card" key={String(n)}><small className="text-zinc-400">{n}</small><h2 className="mt-2 text-2xl font-black text-white">{money(Number(v))}</h2></article>)}</section>
