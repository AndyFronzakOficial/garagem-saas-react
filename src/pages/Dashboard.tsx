...
export default function Dashboard(){
 ...
 return <div>
  ...
  <section className="grid gap-5 xl:grid-cols-2"><div className="card"><h2 className="mb-4 text-2xl font-black">Top Clientes</h2>{s.topClients.map(([name,value]:any)=><div className="mb-3 flex justify-between rounded-xl bg-black/20 p-3"><span>{name}</span><strong>{money(value)}</strong></div>)}{s.topClients.length===0&&<p className="text-zinc-400">Sem dados no mês.</p>}</div><div className="card"><h2 className="mb-4 text-2xl font-black">Top Serviços</h2>{s.topServices.map(([name,count]:any)=><div className="mb-3 flex justify-between rounded-xl bg-black/20 p-3"><span>{name}</span><strong>{count}x</strong></div>)}{s.topServices.length===0&&<p className="text-zinc-400">Sem dados no mês.</p>}</div></section>
 </div>
}
