...
export default function Prices(){
  ...
  return <div>
    ...
    <div className="card table-wrap"><table><thead><tr><th>Serviço</th><th>Terceiro</th><th>Cliente final</th><th>Status</th><th>Ações</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td><strong>{r.name}</strong></td><td>{money(r.price_m2_partner)}</td><td>{money(r.price_m2_final)}</td><td><span className={`badge ${r.active?'success':'danger'}`}>{r.active?'Ativo':'Inativo'}</span></td><td><div className="flex flex-wrap gap-2"><button className="btn-gold" onClick={()=>edit(r)}>Alterar</button><button className="btn-dark" onClick={()=>toggle(r)}>{r.active?'Desativar':'Ativar'}</button><button className="btn-red" onClick={()=>remove(r.id)}>Remover</button></div></td></tr>)}{rows.length===0&&<tr><td colSpan={5} className="text-zinc-400">Nenhum preço cadastrado.</td></tr>}</tbody></table></div>
  </div>
}
