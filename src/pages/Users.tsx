...
export default function Users(){
  ...
  return <div>
    ...
    <form onSubmit={save} className="card mb-6 grid gap-3 md:grid-cols-4">
      ...
      <select className="input" value={f.role} onChange={e=>setF({...f,role:e.target.value})}>{roles.map(r=><option key={r}>{r}</option>)}</select>
    ...
    <div className="card table-wrap">
      <table>
        ...
        <tbody>{rows.map(r=><tr key={r.id}><td>{r.name}</td><td>{r.email}</td><td><select className="input max-w-xs" value={r.role||'Funcionário'} onChange={e=>update(r.id,e.target.value)}>{roles.map(role=><option key={role}>{role}</option>)}</select></td></tr>)}</tbody>
      </table>
    </div>
  </div>
}
